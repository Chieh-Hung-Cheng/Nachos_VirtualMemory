...
Thread::Yield ()
{
    ...
    // if there's no next to run: keep running current thread
...
//N2
static void PsudoExecute(){
	Thread* t =kernel->currentThread;
	while(t->burstTime>0){
		t->burstTime --;
		cout<<"EXECUTING "<<t->getName()<<" WITH BURST TIME "<<t->burstTime<<endl;
...
Thread::SelfTest()
{
    ...
    SimpleThread(0);*/
    // N2: Thread Test Here
    char* psudoName[4] = {"Pusdo 1", "Pusdo 2", "Pusdo 3", "Pusdo 4"}; 
	int psudoPriority[4] = {100, 0, 61, 200};
	int psudoBurst[4] = {10, 5, 15, 8};
	
    for(int n=1; n<=4; n++){
		// simulate four threads
		Thread* t = new Thread(psudoName[n-1]);
		t->priorityLevel = psudoPriority[n-1];
		t->burstTime = psudoBurst[n-1];

		t->Fork((VoidFunctionPtr) &PsudoExecute, (void *)NULL);
		cout << "PsudoThread " << n << " is executing." << endl;
	}
